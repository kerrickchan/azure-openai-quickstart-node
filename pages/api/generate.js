...
export default async function (req, res) {
  ...
  try {
    ...
    // Azure OpenAI
    const { id, created, choices, usage } = await client.getCompletions(deploymentId, [generatePrompt(animal)]);
    ...
    if (error.response) {
      ...
      // Azure OpenAI
      console.error(`Error with Azure OpenAI API request: ${error.message}`);
      res.status(500).json({
        error: {
